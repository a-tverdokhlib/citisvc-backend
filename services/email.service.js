...
/**
 * Send an booking state email
 * @param {string} to
 * @param {string} status
 * @returns {Promise}
 */
const sendBookingStateEmail = async (to, status) => {
    const subject = `Your booking request ${status}.`
    const text = `Hello, <BR>
    Your booking request now ${status}. Please check your bookings. If you're having trouble ...`;
